...
import { poll } from '@/sections/PollSection.data';
...
const POLL_KEY = `qn:poll:${poll.id}`;

export default function PollSection() {
  const sectionRef = useRef<HTMLElement>(null);
  const cardRef = useRef<HTMLDivElement>(null);
  const [voteRecord, setVoteRecord] = useLocalStorage<VoteRecord | null>(POLL_KEY, null);
  const [selectedOption, setSelectedOption] = useState<string | null>(null);
  const [showResults, setShowResults] = useState(voteRecord !== null);
  ...
  const handleVote = () => {
    ...
    setVoteRecord({ optionId: selectedOption, votedAt: new Date().toISOString() });
    setShowResults(true);
  };
  ...
  return (
    ...
    >
      <div ref={cardRef} className="max-w-2xl mx-auto paper-card rounded-2xl overflow-hidden">
        ...
        <div className="p-6 sm:p-8">
          ...
          {!showResults ? (
            <div className="space-y-3">
              {poll.options.map((option) => (
                ...
                >
                  ...
                  >
                    {selectedOption === option.id && <Check className="w-3 h-3 text-cream" />}
                  </div>
            ...
            <section aria-label="Poll results" className="space-y-4">
              <p className="text-sm font-medium text-vintage-red">Poll Results</p>

              {poll.options.map((option) => {
                const votes = option.votes ?? 0;
                const percentage = totalVotes > 0 ? Math.round((votes / totalVotes) * 100) : 0;
                const isSelected = (voteRecord?.optionId ?? selectedOption) === option.id;

                return (
                  <div key={option.id} className="relative">
                    <div className="flex items-center justify-between mb-1">
                      <span
                        className={`font-medium ${isSelected ? 'text-vintage-red' : 'text-espresso'}`}
                      >
                        {option.label}
